...
exports.createTransaction = async (req, res) => {
  try {
    const userId = req.user.id || req.user.user_id;
    const { title, type, amount, merchant, category_id, date, transaction_date, parsedText } = req.body || {};
    ...
    const newTransaction = {
      ...
      category_id: transactionService.resolveCategoryId({ category_id, merchant, parsedText }), // ← เปลี่ยนตรงนี้
...
const { classifyCategory } = require('../services/categoryService'); // เพิ่มบรรทัดนี้บนสุดของไฟล์
...
exports.scanReceipt = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      categoryId: categoryId,           // ← เพิ่มใหม่ ส่งไปให้ frontend
      categoryConfidence: confidence,   // ← เผื่ออยากโชว์ badge "แนะนำ" ใน UI ทีหลัง
    });
  } catch (err) {
